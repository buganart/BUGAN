...
class GAN(pl.LightningModule):
    def __init__(self, config, trainer, save_model_path):
        ...
        self.save_model_path = save_model_path
        #create components
        generator = Generator(config.g_layer, config.z_size, config.array_size, config.gen_num_layer_unit)
    ...
    def configure_optimizers(self):
        ...
        if config.gen_opt == "Adam":
            self.generator_optimizer = optim.Adam(generator.parameters(), lr=config.g_lr)
        else:
    ...
    def training_step(self, dataset_batch, batch_idx, optimizer_idx):
        ...
        if optimizer_idx == 0:
            ...
            z = torch.randn(batch_size, config.z_size).float().to(device) #128-d noise vector
